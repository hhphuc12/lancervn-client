...
export function fetchListAuctionsByCategory(cateId, cateName, pageNumber = 1) {
    const url = cateName === '全て' ? `${BASE_URL_TOKUBUY}auction/categories/all?page=${pageNumber}` : `${BASE_URL_TOKUBUY}auction/category/${cateId}?page=${pageNumber}`;
    return request.get(url);
}
...
export function fetchListAuctionsByKeyword(keyword, pageNumber) {
    const url = keyword === '' ? `${BASE_URL_TOKUBUY}auction/categories/all?page=${pageNumber}` : `${BASE_URL_TOKUBUY}search?keyword=${keyword}&page=${pageNumber}`;
    return request.get(url);
}
